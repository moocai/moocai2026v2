                  ...
                  {topic.lessons?.map(lesson => {
                    ...
                    <Box key={lesson.id} onClick={() => onNavigate(`/courses/${course.id}/${lesson.id}`)} sx={{ p: 1, borderRadius: '8px', cursor: 'pointer', '&:hover': { bgcolor: 'action.hover' }, minWidth: 0 }}>
                      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                        ...
                        {dbProgress[`${course.id}_${lesson.id}`] ? (<CheckCircle2 size={16} color={theme.palette.success.main} />) : (<PlayCircle size={16} color={isLastActive ? '#ff9800' : theme.palette.primary.main} />)}
                      </Box>
                    </Box>
                    );
                  })}
